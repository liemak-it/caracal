  ...
  module Renderers
    class DocumentRenderer < XmlRenderer
      ...
      def render_listitem(xml, model, list_num)
        ls      = document.find_list_style(model.list_item_type, model.list_item_level, model.list_item_numbering_text)
      ...
      def render_textfieldgroup(xml, model)
        xml['w'].p paragraph_options do
          model.text_field_models.each do |text_field_model|
            render_textfield(xml, text_field_model)
          end
        end
      end
